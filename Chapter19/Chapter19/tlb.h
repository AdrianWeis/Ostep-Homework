...
long measurePagingMinMax(long PAGESIZE, int NUMPAGES, int LOOPS)
{
    ...
    long *timeArray = (long*) malloc(NUMPAGES * LOOPS * sizeof(long));
    ...
    average = average/(LOOPS*NUMPAGES);
    if(max-min < 10) 
    {
        return average;
    }
    ...
    for(int j = 0; j < LOOPS*NUMPAGES; j++)
    {
    ...
    if(LOOPS*NUMPAGES - counter > 0) {
        result += resultTemp/(LOOPS - counter);
    }
...
long measureOverheadMinMax(int LOOPS) 
{
    long counter = 0;
    double resultTemp = 0;
    long min = 0;
    ...
    long *timeArray = (long*) malloc(LOOPS * sizeof(long));
    assert(timeArray != NULL);
